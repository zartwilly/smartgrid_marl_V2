# -*- coding: utf-8 -*-
"""
Created on Sun May 23 14:50:05 2021
...
# _____________________________________________________________________________
#
#               plot the virtuel and real gain B, BB ---> debut
# _____________________________________________________________________________
def OLD_plot_Bi_BBi(df_ra_pr, rate, price, scenario):
    cols_2_group = ["algo","gamma_version"]
    cols_2_group = ["algo"]
    ...
    #x = list(map(tuple,list(df_res[cols_2_group].values)))
    x = df_res["algo"].values.tolist()
    px = figure(x_range= x, #FactorRange(*x), 
                y_range=(0, df_res[new_cols].values.max() + 5), 
                plot_height = int(350), 
                plot_width = int(WIDTH*MULT_WIDTH), tools = TOOLS, 
                toolbar_location="above")
    ...
    title = "comparison Gamma_version ({},rate:{}, price={})".format(scenario, rate, price)
    px.title.text = title
    px.y_range.start = df_res.B_mean.min() - 1 if df_res.B_mean.min() < 0 else 0
    px.x_range.range_padding = width
    px.xgrid.grid_line_color = None
    px.legend.location = "top_right" #"top_left"
    px.legend.orientation = "horizontal"
...
def plot_Bi_BBi(df_ra_pr, rate, price, scenario):
    cols_2_group = ["algo","gamma_version"]
    cols_2_group = ["algo"]
    ...
    d = {"BB": "real_benefice", "B": "virtual_benefice"}
    new_name_dict = dict()
    for old_name in new_cols:
        new_name = None
        if old_name.split("_")[0] == "BB":
            new_name = d["BB"]
        elif old_name.split("_")[0] == "B":
            new_name = d["B"]
        new_name_dict[old_name] = new_name + "_mean"
    df_res.rename(columns=new_name_dict, inplace=True)
    new_cols = list(new_name_dict.values())
    ...
    data = dict(x = x, 
                real_benefice_mean=df_res.real_benefice_mean.tolist(), 
                virtual_benefice_mean=df_res.virtual_benefice_mean.tolist(), 
                # real_benefice_std=df_res.real_benefice_std.tolist(), 
                # virtual_benefice_std=df_res.virtual_benefice_std.tolist(), 
                # real_benefice_amin=df_res.real_benefice_amin.tolist(), 
                # virtual_benefice_amin=df_res.virtual_benefice_amin.tolist(), 
                # real_benefice_amax=df_res.real_benefice_amax.tolist(), 
                # virtual_benefice_amax=df_res.virtual_benefice_amax.tolist()
                )
    ...
    title = "comparison Gamma_version ({},rate:{}, price={})".format(scenario, rate, price)
    px.title.text = title
    px.y_range.start = df_res.real_benefice_mean.min() - 1 \
                        if df_res.real_benefice_mean.min() < 0 else 0
    px.x_range.range_padding = width
    px.xgrid.grid_line_color = None
    px.legend.location = "top_right" #"top_left"
    px.legend.orientation = "horizontal"
...
def plot_comparaison_gamma_version_Bi_BBi(df_B_C_BB_CC_RU_M):
    """
    plot B, BB
    """
    rates = df_B_C_BB_CC_RU_M.rate.unique(); rates = rates[rates!=0].tolist()
    ...
    for rate, price, scenario in it.product(rates, prices, scenarios):
        ...
        pxs_pr_ra_sc = plot_Bi_BBi(df_ra_pr, rate, price, scenario)
        pxs_pr_ra_sc.legend.click_policy="hide"
...
# _____________________________________________________________________________
#
#               plot the virtuel and real gain B, BB ---> fin
# _____________________________________________________________________________    
...
def DBG_group_plot_on_panel(df_B_C_BB_CC_RU_M, 
    ...
    rows_Bi_BBi = plot_comparaison_gamma_version_Bi_BBi(df_B_C_BB_CC_RU_M)
    tab_evol_Bi_BBi = Panel(child=rows_Bi_BBi, 
                            title="comparison real and virtual gains B, BB")
    print("comparison real and virtual gains  B,BB : Terminee")
